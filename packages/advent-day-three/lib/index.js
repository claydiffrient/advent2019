...
function getDistance(wireOne, wireTwo) {
    ...
    wireOneSplit.forEach(x => {
        ...
        let tracker = 1;
        let lastPoint = wireOnePoints[wireOnePoints.length - 1];
        if (command === "U") {
    ...
    const wireTwoPoints = [startPoint];
    ...
    wireTwoSplit.forEach(x => {
        const { command, distance } = interpretCommandString(x);
        let tracker = 1;
        let lastPoint = wireTwoPoints[wireTwoPoints.length - 1];
        if (command === "U") {
            while (tracker <= distance) {
                const point = [lastPoint[0], lastPoint[1] + tracker];
    ...
    let intersections = wireOnePoints.filter(p1 => wireTwoPoints.some(p2 => p1[0] === p2[0] && p1[1] === p2[1]));
    ...
    const sumStepsTaken = intersections.map(i => {
        ...
        return ((wireOneStepsTakenPerPoint.get(stringI) || 0) +
