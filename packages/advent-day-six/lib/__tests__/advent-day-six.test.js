...
describe("getOrbitCount", () => {
    it("returns proper for the intial example", () => {
        ...
        expect(index_1.getOrbitCount(input)).toBe(42);
    });
});
describe("findCommonAncestor", () => {
    it("finds the common ancestor of two nodes", () => {
        ...
        const comNode = index_1.buildTree(input);
        if (!comNode) {
        ...
        if (hNode && lNode && bNode) {
            expect(index_1.findCommonAncestor(hNode, lNode)).toStrictEqual(bNode);
        }
    });
});
describe("Node", () => {
    describe("constructor", () => {
        it("creates a Node with a string name and empty child array", () => {
            const testNode = new index_1.Node("my name");
    ...
    describe("addChild", () => {
        it("adds a child node", () => {
            const testNode = new index_1.Node("a");
            const testNodeTwo = new index_1.Node("b");
    ...
    describe("findChildlessNodes", () => {
        it("finds all childless nodes", () => {
            ...
            const testNodeTwo = new index_1.Node("b");
            testNode.addChild(testNodeTwo);
            expect(index_1.findChildlessNode(testNode)).toStrictEqual([testNodeTwo]);
        });
    });
    describe("countEdges", () => {
        let rootNode;
        ...
        let indirectChild;
        beforeEach(() => {
            rootNode = new index_1.Node("COM");
            directChild = new index_1.Node("D");
            indirectChild = new index_1.Node("I");
    ...
    describe("countEdgesUntil", () => {
        it("finds the distance between two points", () => {
            ...
            const lNode = comNode.findChild("L");
            if (hNode && lNode) {
                const common = index_1.findCommonAncestor(hNode, lNode);
                if (common) {
    ...
    describe("countPathBetween", () => {
        ...
        if (sanNode && youNode) {
            ...
            if (sanNodeParent && youNodeParent) {
                expect(index_1.countPathBetween(sanNodeParent, youNodeParent)).toBe(4);
            }
        }
    });
    describe("findChild", () => {
        ...
        it("finds an indirect node next to a node with no children", () => {
            let noChildNode = new index_1.Node("NK");
            let nodeToFind = new index_1.Node("F");
            let otherNode = new index_1.Node("T");
